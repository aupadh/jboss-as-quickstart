...
/**
 * <p>
 * An example how to use the new features 'scoped client' introduced with EJBCLIENT-34 in AS7.2.0 or EAP6.1.<br/>
 * If a server without that feature is used the outbound-connection will be used and the behavior is different.
 * </p>
 * <p>
 * The functionality will be the same as MainAppBean provide, the interface @{link MainApp} will be shared to use both.<br/>
...
public class MainEjbClient34AppBean implements MainApp {
  ...
  /**
   * Invoke the AppOne with the scoped client context. The initial connection will use the 'quickuser1', to differentiate the
   * cluster connection it will be use the user 'quickuser2' to see if the clustered context is used or not.
   * 
  ...
  private String invokeAppOne(String text) {
    ...
    try {
      // this context will not use the server configured 'outbound-connection' and also did not use the jboss-ejb-client.xml.
   ...
   * 
   * @param iCtx context to close, <code>null</code> will be ignored. 
   */
  ...
  /**
   * Invoke the App2 with different ejb-client context. The server AppTwoA will be called with the user quickuser1. The server
   * AppTwoB will be called with the user quickuser2. Both invocations are separate, there will no mix between. Also the
   * outbound-connection is not used.
   * 
   * @param text Simple text which will be logged at server side.
   * @return simple collection of the returned results
   */
