 ...
 * 
 */
public class MultiAppClusterJBossApiClient {
  private static final Logger LOGGER = Logger.getLogger(MultiAppClusterJBossApiClient.class.getName());
  ...
  private AppTwo appTwoProxy;

  static {
  }

  public MultiAppClusterJBossApiClient(Boolean secured, boolean useDefaultUser, String user, String password, Boolean debug) {
    ...
    Logger.getLogger(MultiAppClusterJBossApiClient.class.getPackage().getName()).setLevel(
  ...
  private void createContext() {
    ...
    p.put("remote.connections", "appOne,appTwo");
    ...
    p.put("remote.connection.appTwo.port", String.valueOf(5147));
    ...
    if (this.user != null) {
      ...
      p.put("remote.connection.appTwo.username", this.user);
      p.put("remote.connection.appTwo.password", this.password);
  ...
  private void initialize() throws NamingException {
    ...
    this.appOneProxy = lookup("ejb:appone/ejb//AppOneBean", AppOne.class);
    this.appTwoProxy = lookup("ejb:apptwo/ejb//AppTwoBean", AppTwo.class);
  }

  private void invoke1() throws NamingException {
  ...
  private void invoke2() {
    ...
    try {
      // invoke the unsecured method if needed
      if (this.secured == null || Boolean.FALSE.equals(this.secured)) {
        final String result = this.appTwoProxy.invoke("Client call at " + new Date());
        LOGGER.info("The unsecured EJB call returns : " + result);
      }

      // invoke the secured method if wanted
      if (this.secured != null) {
        LOGGER.info("The secured EJB call returns : " + this.appTwoProxy.invokeSecured("Client call secured at " + new Date()));
      }
    } catch (Exception e) {
      LOGGER.log(Level.WARNING,"Invocation failed : "+this.appTwoProxy,e);
    }
  }

  
  private <T> T lookup(String theBeanName, Class<T> theClass) {
    String name = theBeanName + "!" + theClass.getName();

    try {
      return (T) this.context.lookup(name);
    } catch (Exception e) {
      throw new RuntimeException("cannot find EJB by name '"+name+"'", e);
    }
  }
  ...
  public static void main(String[] args) throws NamingException {
    ...
    MultiAppClusterJBossApiClient client = new MultiAppClusterJBossApiClient(secured, useDefaultUser, user, passwd, debug);
